...
def encode_proof_data(provider, block_number, proof_params):
    block_number = (
        block_number
        if block_number == "latest" or block_number == "earliest"
    ...
    proofs_blob = rlp.encode(
        [pool_acct_proof, steth_acct_proof] + pool_storage_proofs + steth_storage_proofs
    )
...
def request_block_header(provider, block_number):
    ...
    if "proofOfAuthorityData" in block_dict:
        block_dict = dict(
            apply_key_map({"proofOfAuthorityData": "extraData"}, block_dict)
        )
...
def request_account_proof(provider, block_number, address, slots):
    ...
    r = provider.make_request(
        "eth_getProof", params=[address.lower(), hex_slots, to_0x_string(block_number)]
    )
